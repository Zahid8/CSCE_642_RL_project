...
from dm_control import viewer  # Imported for rendering
...
class StepLoggingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        obs = self.locals.get('obs_tensor')  # Access the new observations
        ...
        if rewards is not None and obs is not None:
            for i in range(len(rewards)):               
                ...
                if dones[i]:
                    ...
                    print(f"Episode {self.step_num}: Total Reward = {self.episode_reward:.2f}")
...
class SoccerKickEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Define action space based on the environment's action specification
...
evaluate_agent(model, vec_env, num_episodes=10)
