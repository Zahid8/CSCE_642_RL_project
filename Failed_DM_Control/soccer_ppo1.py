...
class DmControlSoccerGymWrapper(gym.Env):
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            img = self.env.physics.render(width=400, height=300, camera_id=0)
            
            # Convert RGB to BGR for OpenCV
            img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            
            # Display the image in a window
            cv2.imshow("DmControl Soccer", img_bgr)
            
            # Wait for 1 ms to allow the image to be rendered
            if cv2.waitKey(1) & 0xFF == ord('q'):
                self.close()
        else:
            # Optionally, return the RGB array for other modes
            return self.env.physics.render(width=800, height=600, camera_id=0)
...
def evaluate(model_path, num_eval_episodes):
    ...
    # Initialize the evaluation environment (non-vectorized)
    eval_env = DmControlSoccerGymWrapper(team_size=1, time_limit=10.0, walker_type=dm_soccer.WalkerType.HUMANOID)
...
def main():
    ...
    parser.add_argument('--num_timesteps', type=int, default=100_000, help='Number of timesteps to train.')
